...
void handleSignal(int) { g_shouldStop = true; }
...
int main(int argc, char** argv)
{
  ...
  rosmon::launch::LaunchConfig::OutputAttr outputAttrMode =
  ...
  else if (firstArg - optind == 2)
  {
    ...
    {
      fmt::print(
          stderr, "Could not find launch file '{}' in package '{}'\n", fileName, packageName);
      return 1;
    }
  ...
  std::string nodeLogPath;
  {
    ...
    else
    {
      if (logFile.empty())
      {
        fmt::print("Tip: use --log=syslog or set environment variable ROSMON_SYSLOG=1 to send log "
                   "output to syslog instead of a log file in /tmp.\n");
  ...
  {
    monitor.logMessageSignal.connect(boost::bind(logToStdout, _1, get_max_node_name(monitor)));
  }
  ...
  while (!monitor.allShutdown() &&
         ros::WallTime::now() - start < ros::WallDuration(monitor.shutdownTimeout()))
  {
    watcher->wait(waitDuration);
  ...
  bool coredumpsAvailable = std::any_of(monitor.nodes().begin(),
                                        monitor.nodes().end(),
                                        [](const rosmon::monitor::NodeMonitor::Ptr& n)
                                        { return n->coredumpAvailable(); });
  ...
  if (ui && coredumpsAvailable)
  {
    ...
    for (auto& node : monitor.nodes())
    {
      ...
      {
        ui->log({"[rosmon]", fmt::format("{:20}: # {}", node->name(), node->debuggerCommand())});
      }
    }
  }
