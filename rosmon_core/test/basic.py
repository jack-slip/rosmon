...
class BasicTest(unittest.TestCase):
    ...
    def get_param(self, name):
        try:
            ...
            params = ", ".join(
                ['"' + name + '"' for name in sorted(rospy.get_param_names())]
            )
            raise KeyError(
                'Caught KeyError("%s") while getting parameter. Known parameters: %s'
                % (e, params)
            )

    def test_rosmon_running(self):
        try:
            state = rospy.client.wait_for_message(
                "/rosmon_uut/state", State, timeout=5.0
            )
        except rospy.ROSException:
            self.fail(
                "Did not get state msg on /rosmon_uut/state"
                + repr(rospy.client.get_published_topics())
            )
    ...
    def test_params(self):
        self.assertEqual(
            self.get_param("path_to_rosmon"), rospack.get_path("rosmon_core")
        )
        self.assertEqual(
        ...
        self.assert_(
            os.access(executable, os.X_OK), "Invalid rosmon path: " + executable
        )
